			...
			protected Object invoke() {
				...
				if(url!=null && !url.isEmpty()){
					...
					LauncherParams pmtsToOpenView = new LauncherParams.LauncherParamsBuilder(
							"com.sabre.tn.redapp.example.showcase.view.WebKitSampleView", 
					...
					new WorkbenchUtils().openBrowserView("com.sabre.tn.redapp.example.showcase.redapp.webkitview.command", pmtsToOpenView);

					//OpenBrowserViewHandler.showView("com.sabre.tn.redapp.example.showcase.view.WebKitSampleView",url);
				}else{
		...
		new UiThreadInvoker<Object>() {
			...
			protected Object invoke() {
				...
				LauncherParams pmtsToOpenEditor = new LauncherParams.LauncherParamsBuilder(
						"com.sabre.tn.redapp.example.showcase.webkiteditor.command", 
						"Advanced WebBrowser Editor", 
						Activator.PLUGIN_ID)
					.url(url)
					.build();
				
				new WorkbenchUtils().openBrowserEditor("com.sabre.tn.redapp.example.showcase.webkiteditor.command", pmtsToOpenEditor);
