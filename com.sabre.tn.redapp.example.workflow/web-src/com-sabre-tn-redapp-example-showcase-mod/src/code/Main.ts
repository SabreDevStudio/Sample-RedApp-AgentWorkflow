...
import { RSResultArea } from "./views/renderRS/RSResultArea";
...
import { DetailsDrawerConfig } from 'sabre-ngv-core/configs/drawer/DetailsDrawerConfig';
...
export class Main extends Module {
    ...
    init(): void {
        ...
        */
        ...
        drawer.addConfig('search-result',drawerCfg as DetailsDrawerConfig<AbstractView<AbstractModel>, AbstractView<AbstractModel>>); 
    ...
    private showHelperFlows() {
       ...
       // getService(CustomXTPointService).fetchServiceData(new CustomSvcRQData(rq)).done(this.afterGetFlows.bind(this));
        ...
        new Promise( (resolve) => {
            let getFlows: NgvPromise<CommandFlow> = cf('NGV://REDAPP/SERVICE/COM.SABRE.TN.REDAPP.EXAMPLE.SHOWCASE.XTPOINTSERVICES.INTERFACES.ICUSTOMSVC:EXECUTE')
    ...
    private createPNR(): void {
        ...
        let pl = '<GetReservationRQ Version="1.19.0" xmlns="http://webservices.sabre.com/pnrbuilder/v1_19">'+
        ...
        getService(CustomXTPointService).callSOAP(pl,"GetReservationRQ").
        then((res)=>{
            console.log("SOAP result");console.log(res);

            let dp = new DOMParser();
            let rsObj = JSON.parse(res);
            console.log("res.result");
            console.log(rsObj)
            console.log(rsObj.response.payload.responseText);
            let doc = dp.parseFromString(rsObj.response.payload.responseText,"text/xml");
            ...
            let node = resParse.iterateNext();
            let arrExisting = new Array<string>();
            let mpExisting = new Map<string,number>();
            while(node){
        ...
        }).catch((res)=>{console.log("SOAP result");console.log(res)
        });
        /*
        let pnrOptions = {
            title: 'Add to PNR',
